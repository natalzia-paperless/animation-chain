...
function ChainHandler(options, time) {
  this.ticks = [];
...
ChainHandler.prototype = {
  ...
  initWithSelector: function(options) {
    try {
      ...
      if (this.ticks.length === 1) {
        ...
        el.addEventListener(animationType, this.animationFinished.bind(this), false);
      }
    } catch (e) {
      console.log(e);
    }
  ...
  animationFinished: function(e) {
    e.target.removeEventListener(e.type, this.animationFinished);
    this.ticks[0] && this.ticks[0].callback();
